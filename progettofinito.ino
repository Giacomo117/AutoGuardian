...
#define AI_LED_PIN 8
#define BUZZER_PIN 5 // Replace with the actual pin number of the buzzer
...
void listenForBridgeSignal() {
  ...
  while (Serial.available() > 0) {
    ...
    // Check for the start of the signal "&" --> Dangerous zone
    if (incomingChar == '&') {
      // Signal received, trigger the led
      if (!signalReceived) {
        startBuzzerAI();
        signalReceived = true;
    ...
    // Check for the start of the signal "$" --> call from a near device
    if (incomingChar == '$') {
...
void startBuzzerAI() {
  digitalWrite(AI_LED_PIN, HIGH);
}

void stopBuzzerAI() {
  digitalWrite(AI_LED_PIN, LOW);
}
